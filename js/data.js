...
(function(){
  ...
  var state={pool:pool,matches:[],messages:[],news:NEWS.slice(),jobs:JOBS.slice(),notifications:[],reports:[],audit:[]};
  ...
  window.AET.data={
    state:state,
    addMatch:function(o){var id=uid();state.matches.unshift({id:id,userIds:['me',o.uid],other:o,lastMessage:'Nouvelle connexion',updatedAt:Date.now(),unread:0});return id;},
    addMessage:function(m,t,me){var ms=state.messages.find(function(x){return x.matchId===m;})||{matchId:m,messages:[]};if(state.messages.indexOf(ms)<0)state.messages.push(ms);ms.messages.push({fromMe:me!==false,text:t,ts:Date.now()});var match=state.matches.find(function(x){return x.id===m;});if(match){match.lastMessage=t;match.updatedAt=Date.now();match.unread=me?0:(match.unread||0)+1;}},
    pushNotif:function(t){state.notifications.unshift({id:uid(),text:t,time:new Date()});},
    audit:function(a,d){state.audit.unshift({ts:Date.now(),action:a,detail:d});}
  };
})();